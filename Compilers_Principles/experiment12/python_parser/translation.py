...
此模块是简易Python解释器的核心部分，实现了基本的语言特性和执行逻辑。
"""
...
class Function:
    ...
    这个类用于表示一个Python函数，包括它的名称、参数列表和函数体。
    它提供了执行函数体的方法，并能够处理参数传递和局部作用域。

    Attributes:
        name (str): 函数的名称。
        arg_names (list): 参数名列表。
        body (_node): 函数体，表示为一个AST节点。
    """
...
class Translator:
    ...
    这个类是解释器的核心，负责接收一个AST节点，并执行对应的Python代码。
    它处理变量的声明、赋值、表达式的计算以及控制流等。

    Attributes:
        root_node (_node): 根节点，表示要执行的代码的AST。
        variable_table (dict): 变量表，用于存储和查找变量值。
    """
